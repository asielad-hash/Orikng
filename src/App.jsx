...
function InvScreen({T,pendingItem,onPendingClear,elapsed=0,itemsOverride=null,itemEventsOverride=null,evtsData=null,phasesData=null,isArchive=false,kitName=null}) {
  ...
  return(
    <div style={{display:"flex",gap:0,height:"100%",minHeight:0}}>
      ...
      {/* Left drag handle */}
      <div style={{width:4,cursor:"col-resize",background:"transparent",flexShrink:0,position:"relative"}} onMouseDown={e=>{const startX=e.clientX;const startW=leftW;const onMove=ev=>{setLeftW(Math.max(200,Math.min(800,startW+(ev.clientX-startX))));};const onUp=()=>{document.removeEventListener("mousemove",onMove);document.removeEventListener("mouseup",onUp);};document.addEventListener("mousemove",onMove);document.addEventListener("mouseup",onUp);}}><div style={{position:"absolute",inset:"0 1px",background:T.border}}/></div>

      {/* CENTER: Content based on category */}
      <Cd T={T} style={{padding:0,minHeight:0,overflow:"hidden",flex:1}}>
        {activeCat?(<>
        {(()=>{const zi=zoneItems.filter(it=>it.cat===activeCat);const zM=zi.reduce((s,i)=>s+i.loc.m,0);const zP=zi.reduce((s,i)=>s+i.loc.p,0);const zD=zi.reduce((s,i)=>s+i.loc.d,0);const zOpened=activeCat==="pack"?zi.reduce((s,i)=>(ITEM_EVENTS[i.id]||[]).filter(e=>e.type==="opened"&&e.at<=elapsed).length+s,0):0;return(
...
<table style={{width:"100%",borderCollapse:"collapse",fontSize:13,fontFamily:SA}}><thead><tr style={{background:T.card2}}>{["Time","Event","Type",""].map(h=><th key={h} style={{padding:"5px 10px",textAlign:"left",fontSize:10,fontFamily:MO,color:T.muted,textTransform:"uppercase",borderBottom:`1px solid ${T.border}`,borderRight:`1px solid ${T.border}`,fontWeight:600,width:h==="Time"?60:h==="Type"?90:h===""?100:undefined}}>{h}</th>)}</tr></thead><tbody>{evts.map((ev,ei)=>{const ec=ev.type==="baseline"?T.blue:ev.type==="to_patient"?T.purple:ev.type==="to_mayo"?T.teal:ev.type==="to_back"?T.blue:T.green;const label=ev.type==="baseline"?"BASELINE":ev.type==="to_patient"?"→ PATIENT":ev.type==="to_mayo"?"→ MAYO":ev.type==="to_back"?"→ BACK TBL":"RESOLVED";return(<tr key={ei} style={{background:ei%2===0?T.card:T.card2}}><td style={{padding:"5px 10px",borderBottom:`1px solid ${T.border}`,borderRight:`1px solid ${T.border}`,fontFamily:MO,fontSize:12,color:T.muted}}>{ev.t}</td><td style={{padding:"5px 10px",borderBottom:`1px solid ${T.border}`,borderRight:`1px solid ${T.border}`,fontSize:13}}>{ev.note}</td><td style={{padding:"5px 10px",borderBottom:`1px solid ${T.border}`,borderRight:`1px solid ${T.border}`,textAlign:"center"}}><P color={ec} T={T} small filled>{label}</P></td><td style={{padding:"5px 10px",borderBottom:`1px solid ${T.border}`}}>{ev.frame?<div onClick={e2=>{e2.stopPropagation();setLightbox({src:ev.frame.split("#")[0],time:parseFloat((ev.frame.split("t=")[1])||"0"),note:ev.note});}} style={{width:80,height:45,borderRadius:2,overflow:"hidden",border:`1px solid ${T.border}`,cursor:"pointer"}}><video src={ev.frame.split("#")[0]} muted style={{width:"100%",height:"100%",objectFit:"cover"}} onLoadedData={e=>{const t=parseFloat((ev.frame.split("t=")[1])||"0");e.target.currentTime=t;}}/></div>:<span style={{fontSize:11,color:T.muted}}>—</span>}</td></tr>);})}</tbody></table>
            ...
            <div style={{padding:12}}>
              {zoneItems.filter(it=>it.cat===activeCat).map((it,idx)=>{const evts=(ITEM_EVENTS[it.id]||[]).filter(e=>e.at<=elapsed);const col=T[activeCatData.ck];const isSel=selItem===it.id;return(
      ...
      {/* Right drag handle */}
      <div style={{width:4,cursor:"col-resize",background:"transparent",flexShrink:0,position:"relative"}} onMouseDown={e=>{const startX=e.clientX;const startW=rightW;const onMove=ev=>{setRightW(Math.max(200,Math.min(800,startW-(ev.clientX-startX))));};const onUp=()=>{document.removeEventListener("mousemove",onMove);document.removeEventListener("mouseup",onUp);};document.addEventListener("mousemove",onMove);document.addEventListener("mouseup",onUp);}}><div style={{position:"absolute",inset:"0 1px",background:T.border}}/></div>
...
function TlScreen({T,as=5,onScreenChange,elapsed=0,phasesData=null,evtsData=null,liveEvtsData=null,transcriptData=null,itemsOverride=null,itemEventsOverride=null,isArchive=false}) {
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:8,height:"100%",minHeight:0}}>
    ...
    <div style={{display:"flex",gap:0,flex:1,minHeight:0}}>
      ...
      {/* Left drag handle */}
      <div style={{width:4,cursor:"col-resize",background:"transparent",flexShrink:0,position:"relative"}} onMouseDown={e=>{const startX=e.clientX;const startW=tlLeftW;const onMove=ev=>{setTlLeftW(Math.max(200,Math.min(800,startW+(ev.clientX-startX))));};const onUp=()=>{document.removeEventListener("mousemove",onMove);document.removeEventListener("mouseup",onUp);};document.addEventListener("mousemove",onMove);document.addEventListener("mouseup",onUp);}}><div style={{position:"absolute",inset:"0 1px",background:T.border}}/></div>
      ...
      {/* Right drag handle */}
      <div style={{width:4,cursor:"col-resize",background:"transparent",flexShrink:0,position:"relative"}} onMouseDown={e=>{const startX=e.clientX;const startW=tlRightW;const onMove=ev=>{setTlRightW(Math.max(200,Math.min(800,startW-(ev.clientX-startX))));};const onUp=()=>{document.removeEventListener("mousemove",onMove);document.removeEventListener("mouseup",onUp);};document.addEventListener("mousemove",onMove);document.addEventListener("mouseup",onUp);}}><div style={{position:"absolute",inset:"0 1px",background:T.border}}/></div>
